...
from evening.prompt_build import build_evening_global_prompt, build_evening_shard_prompt
...
def merge_sharded(global_summary: str, shard_outputs: list[tuple[str, str, str]]) -> str:
    """shard_outputs: [(stance, summary_part, body_part), ...]"""
...
def run_sharded_synthesize(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    global_prompt = build_evening_global_prompt(ctx)
    global_raw, global_model, global_err = _chat_with_retry(
        EVENING_GLOBAL_SYSTEM,
        global_prompt,
        max_tokens=1200,
        timeout_sec=60.0,
    )
    ...
    def _run_shard(item: tuple[str, str, str, str, float]) -> tuple[str, str, str, dict[str, Any]]:
        ...
        user = build_evening_shard_prompt(ctx, stance)
        shard_system = (
            f"{EVENING_SHARD_SYSTEM}\n\n"
            f"本档推送标签：{push_label}\n本档章标题：## {chapter}"
        )
        raw, model, err = _chat_with_retry(
            shard_system,
            user,
            max_tokens=_max_tokens(tier_stocks),
            timeout_sec=timeout,
        )
    ...
    if failed_critical and cfg.get("synthesize_fallback_monolithic", True):
        ...
        mono = run_monolithic_synthesize(ctx, build_evening_user_prompt(ctx))
