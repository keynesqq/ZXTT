...
def refresh_live_html(day: date, status: dict[str, Any]) -> Path:
    ...
    out_dir = _REPORTS / day.isoformat()
    out_dir.mkdir(parents=True, exist_ok=True)
    path = out_dir / "daily_evening.html"
    atomic_write_text(path, build_evening_live_page(status))
    atomic_write_text(out_dir / "index.html", build_evening_live_page(status))
    return path
