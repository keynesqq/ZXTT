...
def _context_as_of_draft(
    *,
    ...
    if cls_art:
        stamps.append(str(cls_art.get("collected_at_iso") or cls_art.get("updated_at_iso") or ""))
...
def build_evening_bundle(*, on_date: date | None = None) -> dict[str, Any]:
    """读 7 路 raw，合并为 EveningBundle（内存，3.1 不落盘 evening_context）。"""
    ...
    cls_art = _load_json(DATA_DIR / "cls_articles" / f"{iso}.json")
