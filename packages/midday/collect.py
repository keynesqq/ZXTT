...
def _manifest_name(day: date) -> str:
    return f"{day.isoformat()}_midday.json"
...
def _market_sentiment_status(day: date) -> dict[str, Any]:
...
def _index_status(day: date) -> dict[str, Any]:
    try:
...
def _flow_status(day: date) -> dict[str, Any]:
    try:
...
def _write_manifest(payload: dict[str, Any], day: date) -> Path:
    path = _MANIFEST_DIR / _manifest_name(day)
    atomic_write_text(path, json.dumps(payload, ensure_ascii=False, indent=2))
    return path


def run_collect_midday(
    *,
    on_date: date | None = None,
    force: bool = False,
    skip_network: bool = False,
) -> dict[str, Any]:
    """编排 6 路采集并写 collect_manifest。quote 失败则阻断。"""
    t0 = time.monotonic()
    ...
    if skip_network:
        ...
        market_jobs = {
            "market_sentiment": lambda: _market_sentiment_status(trade),
            "market_index": lambda: _index_status(cal),
            "market_flow": lambda: _flow_status(cal),
