...
from core.config import midday_cfg
...
_CTX_DIR = DATA_DIR / "midday_context"
...
def _load_context(day: date) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"midday_context missing: {path}")
    return json.loads(path.read_text(encoding="utf-8"))


def _update_synthesize_manifest(day: date, record: dict[str, Any]) -> None:
    path = _DIGEST_DIR / day.isoformat() / "midday_manifest.json"
    if path.is_file():
        try:
            manifest = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            manifest = {"schema_version": 1, "slot": "midday", "trade_date": day.isoformat(), "stages": {}}
    else:
        manifest = {"schema_version": 1, "slot": "midday", "trade_date": day.isoformat(), "stages": {}}
...
def _run_synthesize(ctx: dict[str, Any]) -> dict[str, Any]:
    mode = str(midday_cfg().get("synthesize_mode") or "monolithic").strip().lower()
    if mode == "sharded":
        return run_sharded_synthesize(ctx)
    user_prompt = build_midday_user_prompt(ctx)
...
def run_midday_ai(
    *,
    ...
    if not raw:
        ...
        out_path = _AI_DIR / f"midday_{cal.isoformat()}.json"
    ...
    exp_path = None
    if body and not err:
        exp_path = extract_and_save_expectations(body, ctx, target_date=cal, missing_codes=missing)
