...
from report.render import build_midday_render_context

_REPORTS = ROOT / "reports"
_LAST = DATA_DIR / "last_midday_report.json"


def run_midday_render(*, on_date: date | None = None) -> dict[str, Any]:
    cal = on_date or date.today()
    rc = build_midday_render_context(on_date=cal)
    html_text = build_midday_page(rc)
    out_dir = _REPORTS / cal.isoformat()
    out_dir.mkdir(parents=True, exist_ok=True)
    main_path = out_dir / "daily_midday.html"
