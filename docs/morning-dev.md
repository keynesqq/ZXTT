...
| AI | **分片并发**（默认）或单次 fallback；`timeout` **110s** |
| SLA | 竞价就绪后 **180s 内**（分片并行）微信送达 |
...
| 总时限 | **180s**（`synthesize_mode: sharded`） |
...
- 默认 **`synthesize_mode: sharded`**：全局摘要 1 次 + 自选板块分片 **并行**（超 `shard_max_stocks` 自动拆批）
- fallback **`monolithic`**：单次合成（易漏股，仅调试）
- `timeout_sec=110`；`synthesize_fallback_monolithic: false`（禁止回退单次）
- **禁止**无分片硬塞 31 只全文
...
| 全局 | 【环境】【超预期】【操作】 | 各 1–2 句；素材在正文按股写，摘要不写【9:15素材】 |

### 10.3 System（`MORNING_SYSTEM`）

1. 集合竞价 vs 昨晚预期；服务 **9:30 后前 30 分钟**。
2. 不重写基本面；无新证据不推翻昨晚。
3. 无新素材须写「自昨晚 22:00 后无新公告/资讯」（写在正文 refresh/no_new 股，非推送摘要块）。
4. 推送：【环境】【超预期】【我的】【想买的】【操作】；**不含**【9:15素材】（微信端亦隐藏该块）。
5. 判定词仅用核对表六档枚举；环境段引用 prompt 中昨涨停今开（若有）；核对只数由页面本地展示，AI 勿重复报数。
...
1. **竞价大盘条**：指数缺口、昨涨停溢价（若有）、核对统计（本地）、AI【操作】首句  
2. **正式报告**：按自选板块 Tab；标题行注入核对表，正文为 AI 短评  
3. （可选扩展）9:15 素材三态、核对主表 — 数据在 `morning_pre` / `morning_checks`，主版 HTML 暂不单独 Tab
